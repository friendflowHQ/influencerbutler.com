...
## How matches are presented: single best or a short list

**When a match is found** controls how many candidates each stale piece offers you. **Let me pick from a short list** (default) shows the top candidates so you can choose the right one yourself. **Suggest the single best match** surfaces only the highest-scoring candidate, which is quicker to clear when the top pick is almost always correct for your catalog. This is the same choice Video Reload Butler offers when it matches international listings by title.
...
- **Also tag the parent product**: on by default. When a match is found, Retag Butler tags both the exact product and its parent listing on the same piece, so the tile covers the specific variation and the whole family. Turn it off if you want only the exact variation tagged. It's skipped on parent-only fixes, where the goal is the reverse.
...
- **Runs forever without scanning, and every Amazon page says the page cannot be found**: your storefront link is still the example one Butler ships with. Open **Settings → Profile** and paste your own link (it looks like `https://www.amazon.com/shop/yourname`), or run the storefront check there to fill it in for you. Paste the link on its own, with no angle brackets around your name: Amazon reads `<yourname>` as a different address and returns the same empty result.
...
- **A run finds only one or two items, and my variant items never get their parent ASIN**: **Run** only looks for items whose product is no longer available on Amazon and suggests a replacement, so items with live products are left alone on purpose. In Review mode nothing changes until you click **Tag this** on a row. To add the parent ASIN to every variant item on your storefront, use the separate **Backfill parent IDs (all items)** button instead.
- **"Backfill parent IDs (all items)" looks stuck on Running**: it is slow by design, not frozen. It opens every piece of content on your storefront in the browser, loads the product pages it needs and edits the post, which is roughly a minute per piece. It also covers your photos and lists, not just your videos, so a small-looking storefront can still be several hundred pieces. It saves its place as it goes, so you can stop it and start it again without losing work.
