...
2. In **Storefront links**, paste your first storefront URL. Give it an optional **Name** (like "Karen's storefront") so you can tell your links apart at a glance. Click **Add another storefront link** to add more, or the **×** on a row to remove it.
3. In **Run settings**:
...
A Chrome window opens, navigates to each storefront, scrolls, and clicks like on eligible content (posts, videos, photos, idea lists, and media lists - all toggleable in **Content types**). It also opens the storefront's Idea Lists and Media Lists tabs when those are enabled. The **Watch it run** checkbox is on by default so you can see what's happening.
