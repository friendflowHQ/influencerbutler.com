...
- **You sign in to Instagram but Butler still says it is not connected**: the Chrome window Butler opens is a plain browser on purpose, so Instagram treats it as a normal sign-in and honours **Trust this device**. Chrome only writes that session to disk when the window closes, so signing in and leaving it open looks exactly like a failed connection. Sign in, tick **Trust this device**, then close that Chrome window completely, not just the tab, and come back to Butler. Opening the sign-in again before closing the previous one starts over, so let one finish.
...
- **Run is going but no DMs send**: if your message has a link that cannot earn you commission, like an Etsy shop or a blog, Butler refuses to send it and notes that in the log. Take the link out, or add the site under **Link Butler > Settings > Allowed links (advanced)** (see the [Link Butler tutorial](/help/tutorials/link-butler)). Your Amazon storefront link always sends.
