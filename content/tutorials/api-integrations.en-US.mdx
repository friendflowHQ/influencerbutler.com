...
Amazon Deals can post your product images and captions straight into a Telegram channel. To wire it up you need two things: a **bot token** and the **channel (chat) ID** you want to post into. Here is the full walkthrough.
...
Back in InfluencerButler, paste the token into the **Telegram Bot Token** field on the API Integrations panel.
...
Once connected, Amazon Deals posts to this channel using Telegram's Bot API `sendPhoto` endpoint. Each post carries the product image from your deal row plus a caption built from your post composer template. Nothing is sent until you publish a post, so connecting the bot is safe to do ahead of time.
