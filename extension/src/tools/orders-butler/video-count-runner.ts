...
type Row = { asin: string; el: HTMLElement; detail: HTMLElement };
...
export function initOrderVideoCounts(marketplace: string): void {
  ...
  section.append(controls, progress, resultsList);
  ...
  async function run(): Promise<void> {
    ...
    try {
      ...
      for (const item of items) {
        rows.set(item.asin, addRow(resultsList, item, cache));
      }
...
function addRow(
  ...
  li.append(detail);
  list.append(li);
  return { asin: item.asin, el: li, detail };
}
