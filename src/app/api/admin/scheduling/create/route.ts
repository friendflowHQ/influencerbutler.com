 ...
 *
 * A recording bot is scheduled whenever the call has a joinable Google Meet room:
 * either a link generated by the Meet button, OR a hand-pasted meet.google.com
 * link (that stays provider 'manual' so cancel does not delete a calendar event
 * we do not own, but is still a real room a bot can join). Only a call with no
 * Meet link stays 'skipped_no_meet'. The customer confirmation email is only sent
...
export async function POST(request: Request) {
  ...
  // A link generated by the Meet button is a real Google Meet room (so cancel can
  // delete it and a recording bot can join); a hand-pasted link is "manual".
  const provider = joinUrl ? (body.meetingProvider === "google_meet" ? "google_meet" : "manual") : null;
  const meetingId = provider === "google_meet" ? ((body.meetingId || "").trim().slice(0, 200) || null) : null;
  ...
  // Schedule a recording bot for any joinable Meet room (generated or a pasted
  // meet.google.com link), exactly like a self-service booking. Only a call with
  // no Meet link is skipped. Best-effort: a failure never fails the confirmed add.
  ...
  if (!shouldScheduleRecordingBot(provider, joinUrl)) {
    ...
    try {
      const bot = await scheduleBot({
        ...
        joinAtISO: startsAtISO,
        botName: "Influencer Butler Notetaker",
        metadata: { bookingId },
      });
      if (bot) { recallBotId = bot.id; recordingStatus = "scheduled"; }
      else recordingStatus = "failed";
    } catch (e) { console.error("[scheduling/create] schedule bot", e); recordingStatus = "failed"; }
  }
  if (recordingStatus !== "none") {
    try { await admin.from("call_bookings").update({ recall_bot_id: recallBotId, recording_status: recordingStatus }).eq("id", bookingId); }
    catch (e) { console.error("[scheduling/create] recording status update", e); }
  }
  ...
  if (body.sendEmail === true) {
    try {
      const emailData: BookingEmailData = {
        ...
        startMs, userEndMs, userTimezone: timezone, topic, joinUrl, recorded: recordingStatus === "scheduled",
      };
