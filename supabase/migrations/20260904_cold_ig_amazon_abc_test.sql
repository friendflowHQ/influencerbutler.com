-- A/B/C opener test for cold Instagram Amazon influencers (three variant sequences).
--
-- Instagram sibling of 20260904_cold_tiktok_amazon_abc_test.sql. The live
-- cold-ig-amazon sequence (Cold Leads: Instagram (Amazon influencers)) opens
-- well but leaks click -> convert, so this seeds three independent 4-step funnels
-- to split fresh cold IG leads three ways and read a winner from real numbers.
-- Each tests a different hypothesis for why the opener does not convert:
--   A (cold-ig-amazon-a)  Short: one benefit, one link, keeps the winning
--                         subject so only the body changes.
--   B (cold-ig-amazon-b)  Proof: mechanism-led ("your order history is a brand
--                         list") + light social proof, new subjects.
--   C (cold-ig-amazon-c)  Low-friction: step 1 asks for a reply, not a
--                         download; escalates to the trial in later steps.
--
-- The original cold-ig-amazon sequence is left untouched and keeps running for
-- its current enrollees. Do NOT also tag these leads cold-ig-amazon, or they
-- would double-enroll into the original. The admin Sequences "Split evenly
-- across the variants" enroll option fans one batch across -a / -b / -c and keeps
-- one person in one variant, so use that rather than tagging thirds by hand.
--
-- Same 0/3/7/14 day cadence as the original. Cold, unsolicited sends go through
-- the compliant marketing sender, which appends the one-click unsubscribe and
-- postal-address footer and honors the suppression list. Do not add either to
-- the body copy.
--
-- All three are created PAUSED. Activate them in Emails > Sequences BEFORE
-- enrolling (cold sequences: activate first, then enroll/tag). Throttled to 150
-- sends/hour, track_opens true (Resend records opens/clicks), send_hour unset so
-- it drips continuously like the original.
--
...
-- ---------------------------------------------------------------------------
-- Variant A: Cold Leads: Instagram Amazon (A: Short)
-- ---------------------------------------------------------------------------
INSERT INTO email_sequences (id, name, status, trigger, sends_per_hour, track_opens, created_by)
VALUES (
  '1a5e0003-0000-4000-a000-00000000000a',
  'Cold Leads: Instagram Amazon (A: Short)',
  'paused',
  '{"kind":"tag_added","tag":"cold-ig-amazon-a"}'::jsonb,
  150,
  true,
...
VALUES
  (
    '1a5e0003-0000-4000-a000-00000000000a', 1, 0,
    'You are already doing the Amazon influencer thing on Instagram',
    'Hi,

It is Liz from The Social Media Posse. I saw your Instagram and the Amazon storefront behind it, so this will be quick.
...
P.S. We run a free group for Amazon and Walmart creators. Come say hi: https://www.facebook.com/groups/influencerbutler'
  ...
  (
    '1a5e0003-0000-4000-a000-00000000000a', 2, 3,
...
Influencer Butler''s Daily Commission Butler checks your sales against open campaigns every day and accepts the ones that match. You keep creating; it keeps claiming.
...
P.S. Free community of Amazon and Walmart creators: https://www.facebook.com/groups/influencerbutler'
  ...
  (
    '1a5e0003-0000-4000-a000-00000000000a', 3, 7,
    'New: your Amazon storefront now copies itself to Walmart',
    'Hi, Liz here with the update I am most excited about.

We just folded Walmart into Influencer Butler. Walmart Repost takes the videos and photos already on your Amazon storefront and republishes them to your Walmart Creator storefront automatically, but only when it is the exact same product, matched by barcode.

Same content you already made, a second storefront earning. No extra editing.
...
P.S. We trade Amazon and Walmart tips daily in the group: https://www.facebook.com/groups/influencerbutler'
  ...
  (
    '1a5e0003-0000-4000-a000-00000000000a', 4, 14,
    'Last note from me',
...
If not, no hard feelings. Either way, keep making great content.
...
-- ---------------------------------------------------------------------------
-- Variant B: Cold Leads: Instagram Amazon (B: Proof)
-- ---------------------------------------------------------------------------
INSERT INTO email_sequences (id, name, status, trigger, sends_per_hour, track_opens, created_by)
VALUES (
  '1a5e0003-0000-4000-a000-00000000000b',
  'Cold Leads: Instagram Amazon (B: Proof)',
  'paused',
  '{"kind":"tag_added","tag":"cold-ig-amazon-b"}'::jsonb,
  150,
  true,
  'elizabethdean30@gmail.com'
)
...
VALUES
  (
    '1a5e0003-0000-4000-a000-00000000000b', 1, 0,
...
P.S. That free group is here if you want to see what they are running: https://www.facebook.com/groups/influencerbutler'
  ...
  (
    '1a5e0003-0000-4000-a000-00000000000b', 2, 3,
...
P.S. Free Amazon and Walmart creator community: https://www.facebook.com/groups/influencerbutler'
  ...
  (
    '1a5e0003-0000-4000-a000-00000000000b', 3, 7,
    'New: your Amazon storefront now copies itself to Walmart',
    'Hi, Liz here.

Newest piece: Walmart. Walmart Repost takes the content already on your Amazon storefront and republishes it to your Walmart Creator storefront automatically, only when the barcode is an exact match, so it is genuinely the same product.

The creators using it are getting a second commission stream out of content they already made. Nothing new to shoot.
...
P.S. We break down Walmart Creator daily in the group: https://www.facebook.com/groups/influencerbutler'
  ...
  (
    '1a5e0003-0000-4000-a000-00000000000b', 4, 14,
    'Closing the loop',
    'Hi, it is Liz, last one from me.

To recap what Influencer Butler does off your existing Amazon setup: pitches the brands behind what you have sold, accepts the campaigns that match your sales, and copies your storefront over to Walmart. All from the order history and content you already have.

Free 14 days, no card, if you want to see it on your own account:
https://www.influencerbutler.com/go/download

Either way, thanks for reading. Keep making great content.
...
-- ---------------------------------------------------------------------------
-- Variant C: Cold Leads: Instagram Amazon (C: Low-friction)
-- ---------------------------------------------------------------------------
INSERT INTO email_sequences (id, name, status, trigger, sends_per_hour, track_opens, created_by)
VALUES (
  '1a5e0003-0000-4000-a000-00000000000c',
  'Cold Leads: Instagram Amazon (C: Low-friction)',
  'paused',
  '{"kind":"tag_added","tag":"cold-ig-amazon-c"}'::jsonb,
  150,
  true,
  'elizabethdean30@gmail.com'
)
...
VALUES
  (
    '1a5e0003-0000-4000-a000-00000000000c', 1, 0,
    'Quick one about your Instagram storefront',
    'Hi,

It is Liz from The Social Media Posse. Real quick, no pitch in this one.

I saw your Instagram and your Amazon storefront and I think you are leaving two easy commission streams on the table: brand campaigns that match what you have already sold, and products you never circled back to pitch.

I put together a 60-second rundown of how creators are automating both. Want me to send it over? Just reply "Instagram" and I will fire it back.
...
P.S. We also run a free group for Amazon and Walmart creators: https://www.facebook.com/groups/influencerbutler'
  ...
  (
    '1a5e0003-0000-4000-a000-00000000000c', 2, 3,
...
P.S. Free Amazon and Walmart creator group: https://www.facebook.com/groups/influencerbutler'
  ...
  (
    '1a5e0003-0000-4000-a000-00000000000c', 3, 7,
    'New: your Amazon storefront now copies itself to Walmart',
    'Hi, Liz here with one new thing worth 30 seconds.

Influencer Butler now copies your Amazon storefront to Walmart. Walmart Repost republishes the content already on your Amazon storefront to your Walmart Creator storefront automatically, only when it is the exact same product by barcode.

Second storefront, same content, no extra work. If you want to try it on your own account it is still free for 14 days:
https://www.influencerbutler.com/go/download
...
P.S. We talk Walmart Creator daily in the group: https://www.facebook.com/groups/influencerbutler'
  ...
  (
    '1a5e0003-0000-4000-a000-00000000000c', 4, 14,
...
And if not, just ignore this and I will leave your inbox alone. Either way, keep making great content.
